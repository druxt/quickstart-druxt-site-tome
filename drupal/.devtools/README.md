...
| `provision` | Install Drupal from committed config (`site-install --existing-config`). Import Tome content and files. Generate OAuth keys. Create a Consumer. |
...
| `DB_FILE` | `/tmp/quickstart-tome-drupal-site.sqlite` | SQLite database path |
